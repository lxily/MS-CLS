...
namespace Analyze{
    ...
    size_t myGetdelim(char **linep, size_t *n, int delim, FILE *fp){
        int ch;
        size_t i = 0;
        if(!linep || !n || !fp){
            errno = EINVAL;
    ...
    size_t myGetline(char **linep, size_t *n, FILE *fp){
        return myGetdelim(linep, n, '\n', fp);
    }

    // Creates a tile
    tile *get_tile(struct prism *p, int pid, int prev_tiles, int *coord)
    {
        tile *prism_tile = (tile *)calloc(1, sizeof(tile));
    ...
    size_t get_next_line(char **line_ptr, size_t *len, FILE *input)
    {
        size_t read;
        while ((read = myGetline(line_ptr, len, input)) != -1)
        {
            char *line = *line_ptr;
    ...
    tile_node **insert_tile_node(tile_tree *tree, tile_node **tiles, int ntiles)
    {
        ...
        for (int i = 0; i < ntiles; i += 1)
        {
            if (curr_children == 0)
            {
                node = (tile_node *)calloc(1, sizeof(tile_node));
    ...
    tile_node **insert_tile(tile_tree *tree, tile **tiles, int ntiles)
    {
        ...
        for (int i = 0; i < ntiles; i += 1)
        {
            ...
            tile_node *leaf = (tile_node *)calloc(1, sizeof(tile_node));
    ...
    {
        tile_tree *tree = (tile_tree *)calloc(1, sizeof(tile_tree));
    ...
    {
        need_adapters = (connectivity *)calloc(1, sizeof(struct connectivity));
        all_connections = (connectivity *)calloc(1, sizeof(struct connectivity));
                    ...
                    for (int k = 0; k < 4; k += 1)
                    {
                        ...
                        if (res_diff == 1)
                        {
                            fatalif(v_count(curr_adj) != (dimension == 2 ? 2 : 4),
                                    "Tile %d alignment is off, adjacent to not equal %d (%d) tile of low to high resolution",
                        ...
                        {
                            fatalif(v_count(curr_adj) != 1, "Tile %d alignment is off, adjacent to more than 1 (%d) tile of same resolution", 
    ...
    void validate_pe_fit()
    {
        ...
        int *pe_cache = (int *)calloc(prob->fabric_shape[0] * prob->fabric_shape[1], sizeof(int));
        fatalif(pe_cache == 0,
                "Unable to allocate PE cache for validation, fabric shape %d X %d",
                prob->fabric_shape[0], prob->fabric_shape[1]);

        // Cache to check used adpaters PEs
        int *adapter_cache = (int *)calloc(prob->fabric_shape[0] * prob->fabric_shape[1], sizeof(int));
    ...
    double score_wires()
    {
        ...
        bool *faces_added = (bool *)calloc(6 * v_count(tile_list), sizeof(bool));
    ...
    void validate_empty_prisms()
    {
        for (int i = 0; i < sol->nprims; i += 1)
        {
            ...
            if (dimension == 2)
            {
				...
				fatalif(abs_d(i2) > DOUBLE_EPSILON, "Prism %d at coord (%d, %d) cannot be empty. It encloses a non-empty target resolution %f", 
					i, (int)scaled.low_bounds[0], (int)scaled.low_bounds[1], i2);
            ...
            else
            {
				...
				fatalif(abs_d(i2) > DOUBLE_EPSILON, "Prism %d at coord (%d, %d, %d) cannot be empty. It encloses a non-empty target resolution %f", 
					i, (int)scaled.low_bounds[0], (int)scaled.low_bounds[1], (int)scaled.low_bounds[2], i2);
            }
        }
    }
    ...
    void readProblem(int &dim,
                     std::vector<int> &cubeShape,
                     std::vector<int> &fabricShape,
        ...
        {
            res = sscanf(line, "%d", &dim);
            fatalif(res != 1 || dim > 3 || dim < 2,
                    "%s: Invalid input for dimension, must be 2 or 3", line);
        }
        else
        {
            fatalif(1, "%s, Dimension missing", line);
        }
        cubeShape.resize(dim);
        ...
        if ((read = get_next_line(&line, &len, stdin)) != -1)
        {
            if (dim == 2)
            {
                res = sscanf(line, "%d%d", &cubeShape[0], &cubeShape[1]);
                fatalif(res != 2 ||
                            cubeShape[0] <= 0 ||
                            cubeShape[1] <= 0,
            ...
            {
                res = sscanf(line, "%d%d%d", &cubeShape[0], &cubeShape[1], &cubeShape[2]);
                fatalif(res != 3 ||
                            cubeShape[0] <= 0 ||
                            cubeShape[1] <= 0 ||
                            cubeShape[2] <= 0,
        ...
        fabricShape.resize(2);

        if ((read = get_next_line(&line, &len, stdin)) != -1)
        {
            res = sscanf(line, "%d%d",
                         &fabricShape[0], &fabricShape[1]);
            fatalif(res != 2 ||
                        fabricShape[0] <= 0 ||
                        fabricShape[1] <= 0,
        ...
        if (dim == 2)
        {
            heatmap_num = (cubeShape[0] + 1) * (cubeShape[1] + 1);

        }
        else
        {
            heatmap_num = (cubeShape[0] + 1) * (cubeShape[1] + 1) * (cubeShape[2] + 1);
        }
        heatMap.resize(heatmap_num);
        ...
        for (int i = 0; i < heatmap_num; i += 1)
        {
            if ((read = get_next_line(&line, &len, stdin)) != -1)
            {
                res = sscanf(line, "%lf",
                             &heatMap[i]);
                fatalif(res != 1 ||
                            heatMap[i] < 0.0f - DOUBLE_EPSILON ||
                            heatMap[i] > 1.0f + DOUBLE_EPSILON,
    ...
    void analyze_prisms(const int &dim,
                        const std::vector<int> &cubeShape,
                        const std::vector<int> &fabricShape,
                        const double &alpha, const double &beta,
                        const std::vector<double> &heatMap,

                        const int &inverse_sampling_step,
                        const std::vector<std::vector<int>> &prisms_info,

                        int &tile_cnt, int &adapter_cnt,
                        std::vector<std::vector<int>> &connections){

        prob = (problem *) calloc(1, sizeof(struct problem));

        dimension = dim;

        if(dim == 2){
            prob->volume_shape[0] = cubeShape[0];
            prob->volume_shape[1] = cubeShape[1];
        }
        else{
            prob->volume_shape[0] = cubeShape[0];
            prob->volume_shape[1] = cubeShape[1];
            prob->volume_shape[2] = cubeShape[2];
        }
        ...
        prob->fabric_shape[0] = fabricShape[0];
        prob->fabric_shape[1] = fabricShape[1];

        prob->cost.alpha = (float)alpha;
        prob->cost.beta = (float)beta;
        ...
        for (int i = 0; i < heatmap_num; i += 1)
        {
            prob->heatmap[i] = (float)heatMap[i];
        }

        // prism_input
        sol = (solution *) calloc(1, sizeof(struct solution));
        sol->inv_sampling_step = inverse_sampling_step;
        ...
        for(int i = 0; i < (int)prisms_info.size(); ++i){
            ...
            if (dimension == 2){
                ...
                p.origin[0] = prisms_info[i][1];
                p.origin[1] = prisms_info[i][2];
                p.shape[0] = prisms_info[i][3];
                p.shape[1] = prisms_info[i][4];
            }
            else{
                p.resolution = prisms_info[i][0];
                p.origin[0] = prisms_info[i][1];
                p.origin[1] = prisms_info[i][2];
                p.origin[2] = prisms_info[i][3];
                p.shape[0] = prisms_info[i][4];
                p.shape[1] = prisms_info[i][5];
                p.shape[2] = prisms_info[i][6];
            }

            fatalif(p.resolution < 0 && p.resolution != -1, "%d: Invalid resolution, should be non negative integer or -1 for empty prisms", p.resolution);
            ...
            for (int i = 0; i < dimension; i += 1)
            {
                fatalif(p.origin[i] < volume_bounds.low_bounds[i] ||
                        ...
                        "Invalid origin coordinate %d in dimension %d, should be within volume bounds [%d,%d]",
                        p.origin[i], i, volume_bounds.low_bounds[i], volume_bounds.up_bounds[i]);

                fatalif(p.shape[i] <= 0, "Invalid shape dimension %d, should be positive integer", i);

                int close = get_close(&p, i);
                fatalif(close > volume_bounds.up_bounds[i] || close < volume_bounds.low_bounds[i],
                        "Shape length %d too large in dimension %d, should be within volume bounds [%d,%d]",
                        close, i, volume_bounds.low_bounds[i], volume_bounds.up_bounds[i]);
            }
...
#ifdef LOCAL
		double scale = find_max_scale();
		printf("[Print in analyze.h] scale = %.3lf\n", scale);
#endif //LOCAL

        neighbour *connection_list = get_adjacent_list(all_connections);
        neighbour *adapter_list = get_adjacent_list(need_adapters);

        tile_cnt = v_count(tile_list);
        nadaptor = v_count(adapter_list);

        connections.resize(tile_cnt);

        for (int i = 0; i < v_count(connection_list); i += 1){
            if(tile_list[connection_list[i].curr_tile]->parent->resolution ==
               tile_list[connection_list[i].adjacent_tile]->parent->resolution){
                connections[connection_list[i].curr_tile].push_back(connection_list[i].adjacent_tile);
            }
        }

        std::vector<int>adapter_connection;
        int pre_tile = -1;
        int pre_face = -1;
        int curr_ada = -1;

        for (int i = 0; i < nadaptor; i += 1){
			//printf("%d, %d, %d\n", adapter_list[i].curr_tile, adapter_list[i].face, adapter_list[i].adjacent_tile);
            if(pre_tile != adapter_list[i].curr_tile || pre_face != adapter_list[i].face){
                if(adapter_connection.size()){
                    connections.push_back(adapter_connection);
                    adapter_connection.clear();
                }

                pre_tile = adapter_list[i].curr_tile;
                pre_face = adapter_list[i].face;
                curr_ada = (int)connections.size();

                adapter_connection.push_back(pre_tile);
                connections[pre_tile].push_back(curr_ada);
            }

            adapter_connection.push_back(adapter_list[i].adjacent_tile);
            connections[adapter_list[i].adjacent_tile].push_back(curr_ada);
        }
        ...
        adapter_cnt = (int) connections.size() - tile_cnt;
